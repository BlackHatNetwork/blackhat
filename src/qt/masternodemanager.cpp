...
static void NotifynodehatUpdated(MasternodeManager *page, CnodehatConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updatenodehat", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifynodehatChanged.connect(boost::bind(&NotifynodehatUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifynodehatChanged.disconnect(boost::bind(&NotifynodehatUpdated, this, _1));
}
...
void MasternodeManager::updatenodehat(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        BOOST_FOREACH(PAIRTYPE(std::string, CnodehatConfig) adrenaline, pwalletMain->mapMynodehats)
        {
            updatenodehat(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditnodehat* aenode = new AddEditnodehat();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CnodehatConfig c = pwalletMain->mapMynodehats[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    nodehatConfigDialog* d = new nodehatConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMynodehats.erase(sAddress);
        walletdb.ErasenodehatConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CnodehatConfig) adrenaline, pwalletMain->mapMynodehats)
    {
        CnodehatConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
