...
AddEditnodehat::AddEditnodehat(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditnodehat)
{
    ui->setupUi(this);

}

AddEditnodehat::~AddEditnodehat()
{
    delete ui;
}


void AddEditnodehat::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMynodehats.insert(make_pair(c.sAddress, c));
	walletdb.WritenodehatConfig(c.sAddress, c);
        uiInterface.NotifynodehatChanged(c);

        accept();
    }
}

void AddEditnodehat::on_cancelButton_clicked()
{
    reject();
}
