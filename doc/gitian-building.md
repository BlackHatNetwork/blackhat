...
*Setup instructions for a gitian build of BlackHat using a Debian VM or physical system.*
...
- [Building BlackHat](#building-blackhat)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-blackhat-host/README.md).
...
Clone the git repositories for blackhat and gitian and then checkout the blackhat version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/blackhat/blackhat
cd blackhat
...
Follow the instructions in [doc/release-process.md](release-process.md) in the blackhat repository
...
To build BlackHat (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the blackhat repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/blackhat/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/blackhatnetwork/blackhat
...
For example:
```bash
URL=https://github.com/laanwj/blackhat.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit blackhat=${COMMIT} --url blackhat=${URL} ../blackhat/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit blackhat=${COMMIT} --url blackhat=${URL} ../blackhat/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit blackhat=${COMMIT} --url blackhat=${URL} ../blackhat/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/blackhat-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/blackhat-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/blackhat-build.assert
```
...
[blackhat/gitian.sigs](https://github.com/blackhat/gitian.sigs/) repository, or if that's not possible create a pull
